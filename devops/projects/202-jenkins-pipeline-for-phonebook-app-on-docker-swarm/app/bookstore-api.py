...
def init_bookstore_db():
    ...
    book_table = """
    CREATE TABLE bookstore_db.books(book_id INT NOT NULL AUTO_INCREMENT, 
...
# Write a function named `get_all_books` which gets all books table in the db,
# and return result as list of dictionary 
# '[{'book_id': 1, 'title':'XXX', 'author': 'XXXXX', 'is_sold': 'YES' or 'NO'}]'.
def get_all_books():
    query = f"""
    SELECT * FROM books;
    """
...
def change_book(book):
    update = f"""
    UPDATE books
    SET title='{book['title']}'. author = '{book['author']}'. is sold = {book['is sold']}
...
# Write a function named `home` which returns 'Welcome to the Salih's Bookstore API Service',
...
# Write a function named `add_books` which adds new book using 'POST' methods,
# and assign to the static route of ('/books')
...
# Write a function named `update_book` which updatess an existing book using 'PUT' method,
# and assign to the static route of ('/books/<int:book_id>')
...
# Write a function named `delete_book` which updatess an existing book using 'DELETE' method,
# and assign to the static route of ('/books/<int:book_id>')
